...
export async function run(sock, msg) {
  ...
  // Comando de ffmpeg con tu configuración original y los metadatos de marca de agua
  exec(`ffmpeg -i "${inputPath}" -vf "scale=512:512,fps=15,format=rgba" -loop 0 -preset default -an -vsync 0 -metadata author="PandaBot™️" -metadata title="PandaSticker" "${outputPath}"`, async (err) => {
    if (err) {
      console.error(err);
