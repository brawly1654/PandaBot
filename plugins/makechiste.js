...
export async function run(sock, msg, args) {
  ...
  // Lógica para aceptar o rechazar un chiste (solo para dueños)
  if (subCommand === 'aceptar' || subCommand === 'rechazar') {
    ...
    jokes.pendingJokes.splice(jokeIndex, 1); // Remover de pendientes

    if (subCommand === 'aceptar') {
      jokes.acceptedJokes.push(joke);
      await sock.sendMessage(from, { text: `✅ Chiste #${jokeId} aceptado y guardado.` });
      await sock.sendMessage(joke.senderJid, { text: `🎉 ¡Tu chiste ha sido aprobado! Ahora se mostrará en *.chisteRandom*.` });
    } else { // Rechazar
      await sock.sendMessage(from, { text: `❌ Chiste #${jokeId} rechazado.` });
      await sock.sendMessage(joke.senderJid, { text: `💔 Tu chiste ha sido rechazado.` });
    }
  ...
  // Lógica para que un usuario envíe un chiste
  ...
  const jokes = loadJokes();
  const newId = (jokes.pendingJokes?.length > 0 ? Math.max(...jokes.pendingJokes.map(j => j.id)) : 0) + 1; // Corregido
...
Para rechazar: *.makechiste rechazar ${newId}*
`;
